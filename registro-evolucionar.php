...
<?php
// backend/mail/registro-evolucionar.php
// Template de email para Plan Evolucionar (llamado desde success.php)
...
try {
    ...
    $mail->isHTML(true);
    $mail->Subject = 'Bienvenido al Plan Evolucionar - ' . $config['app']['name'];
    ...
    if (!$emailSent) {
        error_log('Error enviando email Plan Evolucionar: ' . $mail->ErrorInfo);
    }
    
} catch (Exception $e) {
    error_log('Error en template email Evolucionar: ' . $e->getMessage());
}
?>